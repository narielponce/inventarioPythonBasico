...
def listar_inventario(marca="", categoria="", modelo="", numero_serie=""):
    ...
    query = '''
        ...
        WHERE
            ...
            AND (a.modelo LIKE ? OR ? = '')
            AND (a.serial_number LIKE ? OR ? = '')
    '''
    cursor.execute(query, (f"%{marca}%", marca, f"%{categoria}%", categoria, f"%{modelo}%", modelo, f"%{numero_serie}%", numero_serie))
    inventarios = cursor.fetchall()
    conn.close()
    return inventarios


def listar_inventario_ant():
    conn = sqlite3.connect('deposito.db')
    cursor = conn.cursor()
    cursor.execute('''
            SELECT 
...
def listar_inventario_gui():
    limpiar_pantalla()
    tit = ttk.Label(root, text="Listado Inventario", font=("Century Gothic", 14))
    tit.pack(pady=10)
    ...
    # Extraer solo los nombres de las categorías y marcas
    categorias_nombres = [categoria[1] for categoria in
                          categorias]
    ...
    def exportar_excel():
        ...
        modelo = modelo_entry.get()
        numero_serie = serie_entry.get()
        inventarios = listar_inventario(marca, categoria, modelo, numero_serie)
        ...
        tk.messagebox.showinfo("Exportar a Excel", "El inventario se ha exportado exitosamente a 'inventario.xlsx'.")
    ...
    # Crear el Treeview
    tree = ttk.Treeview(frame, columns=("ID", "Artículo", "Marca", "Categoria", "Modelo", "N° Serie", "Cantidad", "Ubicación"),
                        show='headings')
    tree.heading("ID", text="ID")
    ...
    tree.column("Categoria", width=60, anchor=tk.W)
    ...
    def actualizar_treeview(marca, categoria, modelo, numero_serie):
        ...
        # Agregar los inventarios al Treeview
        for inventario in listar_inventario(marca, categoria, modelo, numero_serie):
            tree.insert("", tk.END, values=inventario, tags=('fontstyle',))

    # Inicializar el Treeview con todos los inventarios
    actualizar_treeview("", "", "", "")

    ttk.Button(root, text="Volver", command=mostrar_menu).pack(pady=20)


def listar_inventario_gui_ant():
    limpiar_pantalla()
    ...
    # Crear el Treeview
    tree = ttk.Treeview(frame, columns=("ID", "Artículo", "Marca", "Modelo", "N° Serie", "Cantidad", "Ubicación"),
                        show='headings')
    tree.heading("ID", text="ID")
    tree.heading("Artículo", text="Artículo")
    tree.heading("Marca", text="Marca")
    ...
    # Agregar los inventarios al Treeview
    for inventario in listar_inventario():
